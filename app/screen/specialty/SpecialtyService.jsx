...
export default function SpecialtyService() {
  ...
  useEffect(() => {
    if (specialtyId) {
      ...
      // console.error('specialtyId is missing!');
    }
  }, [page, specialtyId]);
  ...
  const fetchServices = async () => {
    setLoading(true);
    try {
      const data = await getServiceSpecialty(specialtyId, page, 5);
      setTotalPages(data.totalPages);
      setSpecialtyServices(data.data);
    } catch (error) {
  ...
  return (
    <View style={tw`flex-1 p-4 bg-gray-100`}>
      {loading ? (
        ...
        <>
          <FlatList
            data={specialtyServices}
            keyExtractor={(item) => item.id}
            renderItem={renderServiceSpecialty}
          />
          {renderPagination()}
        </>
      )}
